...
const input =
  "w-full bg-zinc-950/80 border border-zinc-800 rounded-lg px-3.5 py-2.5 text-zinc-100 text-sm focus:outline-none focus:border-teal-500/60 focus:bg-zinc-950 transition-colors placeholder:text-zinc-600";
const labelCls =
  "block text-xs font-medium text-zinc-400 mb-1.5 uppercase tracking-wide";
...
function Toggle({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`w-9 h-5 bg-zinc-700 rounded-full peer ${colors[color]} transition-colors duration-200 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-4`}
      />
    </label>
  );
}
...
export function CommandEditForm({
  command,
  onCancel,
  onSuccess,
}: CommandEditFormProps) {
  const defaultScheduleType: ScheduleType = command.run_at_secs
    ? "datetime"
    : command.interval_secs > 0
      ? "interval"
      : "manual";
  ...
  const defaultIntervalSecs =
    !command.run_at_secs && command.interval_secs > 0
      ? command.interval_secs.toString()
      : "60";
  ...
  const scheduleOptions = [
    {
      ...
      desc: "Execute immediately, one time",
    },
    {
      value: "interval",
      label: "Recurring",
      icon: <Clock className="w-4 h-4" />,
      desc: "Repeat on an interval",
    },
    {
      value: "datetime",
      label: "Scheduled",
  ...
  return (
    <div className="bg-zinc-900/70 border border-zinc-800/80 rounded-xl shadow-2xl overflow-hidden max-w-2xl mx-auto w-full">
      ...
      <div className="px-5 py-4 border-b border-zinc-800/80 flex items-center gap-3">
        ...
        <button
          onClick={onCancel}
          className="text-zinc-400 hover:text-zinc-600 transition-colors p-1 rounded-md hover:bg-zinc-800"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
          ...
          {scheduleType === "interval" && (
            <div>
              ...
              <input
                ...
                min="1"
                className={input}
                {...register("intervalSecs", {
                  required: scheduleType === "interval",
                })}
              />
            </div>
          )}
        ...
        <Section title="Behaviour">
          <ToggleRow
            id="edit-autoStart"
            checked={autoStart}
            onChange={() => setValue("autoStart", !autoStart)}
            color="teal"
            title="Auto-start on launch"
            description="Start this command when the app starts"
          />
          <div className="h-px bg-zinc-800/60" />
          <ToggleRow
            id="edit-notifyOnFailure"
            checked={notifyOnFailure}
            onChange={() => setValue("notifyOnFailure", !notifyOnFailure)}
            color="red"
            title="Notify on failure"
            description="Show a system notification if the command fails"
          />
          <ToggleRow
            id="edit-notifyOnSuccess"
            checked={notifyOnSuccess}
            onChange={() => setValue("notifyOnSuccess", !notifyOnSuccess)}
            color="teal"
            title="Notify on success"
            description="Show a system notification on successful completion"
          />
          <div className="h-px bg-zinc-800/60" />
          <ToggleRow
            id="edit-autoRestartOnFail"
            checked={autoRestartOnFail}
            onChange={() => setValue("autoRestartOnFail", !autoRestartOnFail)}
            color="orange"
            title="Auto retry on failure"
            description="Automatically re-run the command if it fails"
          />
          {autoRestartOnFail && (
            <div className="flex items-center gap-3">
              ...
              <input
                type="number"
                min="1"
                className="w-24 bg-zinc-950/80 border border-zinc-800 rounded-lg px-3 py-1.5 text-zinc-100 text-sm focus:outline-none focus:border-teal-500/60 transition-colors"
          ...
          <div className="h-px bg-zinc-800/60" />
          <ToggleRow
            id="edit-autoRunOnComplete"
            checked={autoRunOnComplete}
            onChange={() => setValue("autoRunOnComplete", !autoRunOnComplete)}
            color="blue"
            title="Loop on completion"
            description="Re-run immediately after each successful completion"
          />
        </Section>
