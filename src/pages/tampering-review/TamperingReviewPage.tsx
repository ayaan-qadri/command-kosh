...
export function TamperingReviewPage() {
  const [tamperedCommands, setTamperedCommands] = useState<
    TamperedCommandInfo[]
  >([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        const state = await invoke<TamperedCommandInfo[]>(
          "get_tampering_state",
        );
        if (state.length === 0) {
          navigate({ to: "/" });
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col items-center justify-center gap-4">
        ...
        <div>
          <p className="text-sm font-medium text-zinc-300 mb-1 text-center">
            Verifying integrity...
          </p>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="p-6">
              ...
              <p className="text-sm text-zinc-400 mt-3 leading-relaxed">
                ...
                command{acceptedCount !== 1 ? "s" : ""}
                {rejectedCount > 0 && (
                  <>
                    {" "}
                    and{" "}
                    <strong className="text-red-400 font-medium">
                      remove {rejectedCount}
                    </strong>{" "}
                    command{rejectedCount !== 1 ? "s" : ""}
                  </>
                )}
