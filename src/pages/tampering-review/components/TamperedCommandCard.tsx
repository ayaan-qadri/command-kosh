...
export function TamperedCommandCard({ cmd, isAccepted, onToggle }: TamperedCommandCardProps) {
    return (
        <div
            onClick={() => onToggle(cmd.id)}
            className={`group relative bg-zinc-900/50 border rounded-xl overflow-hidden cursor-pointer transition-all duration-200 hover:bg-zinc-900/70 hover:shadow-[0_2px_20px_rgba(0,0,0,0.3)] ${isAccepted
        ...
        >
            ...
            <div className="flex items-stretch">
                ...
                <div className="flex items-center px-5 shrink-0">
                    <div
                        className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all duration-200 ${isAccepted
                                ? "bg-emerald-500 border-emerald-400 text-white"
                                : "bg-zinc-800 border-zinc-600 text-transparent group-hover:border-zinc-500"
                            }`}
                    >
                        <Check
                            className={`w-3.5 h-3.5 transition-all duration-200 ${isAccepted ? "scale-100 opacity-100" : "scale-0 opacity-0"
                                }`}
                            strokeWidth={3}
                        />
                    </div>
