    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col font-sans">
      <header className="px-5 py-3.5 border-b border-zinc-800/80 flex items-center justify-between bg-zinc-900/60 backdrop-blur-md sticky top-0 z-10 w-full">
        ...
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-base font-bold bg-gradient-to-r from-teal-400 to-emerald-400 bg-clip-text text-transparent leading-tight">
              Command Kosh
            </h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-md text-sm font-medium transition-all duration-150 border ${
              showForm
      ...
      <main className="flex-1 p-6 z-0 relative">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          {showForm && <CommandForm onSuccess={() => setShowForm(false)} />}

          {!showForm && (
            <CommandList setShowAutostartBanner={setShowAutostartBanner} />
          )}
        </div>
      </main>
    </div>
