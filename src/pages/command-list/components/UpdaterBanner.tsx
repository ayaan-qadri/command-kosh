...
type UpdateStatus = "idle" | "downloading" | "installing" | "error" | "relaunching";
...
export function UpdaterBanner() {
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            if (!mounted.current) return;
            setStatus("error");
            setError(e?.message || String(e) || "Unknown error occurred during update.");
        }
    };

    const isUpdating = ["downloading", "installing", "relaunching"].includes(status);
            ...
            <div className="flex items-center justify-between p-4 flex-col sm:flex-row gap-4">
                <div className="flex items-start gap-4 flex-1">
                    ...
                    <div className="w-full">
                        ...
                        <p className={`text-sm text-teal-400/80 ${isUpdating || status === "error" ? "mb-2" : ""}`}>
                            Version {update.version} is now available. You are currently on an older version.
                        </p>
