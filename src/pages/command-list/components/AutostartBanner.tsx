  ...
  return (
    <div className="bg-amber-500/10 border-b border-amber-500/20 px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4 text-sm animate-in slide-in-from-top-4 z-0 relative">
      <div className="flex items-center gap-3 text-amber-200/90 text-center md:text-left mx-auto md:mx-0">
        <span className="bg-amber-500/20 p-1.5 rounded-full flex-shrink-0">
          <AlertTriangle className="w-5 h-5 text-amber-400" />
        </span>
        <div>
          <p className="font-semibold text-amber-300">Auto-Start is disabled</p>
          <p className="text-amber-200/70 text-sm">
            Some commands require the app to start automatically, but this app
      ...
      <div className="flex flex-wrap justify-center items-center gap-2">
        <button
          onClick={onClose}
          className="px-3 py-1.5 text-zinc-200 hover:text-zinc-400 hover:bg-zinc-800 rounded transition-colors"
        >
          Close
        </button>
        <button
          ...
          onClick={() => {
            enable()
              .then(() => {
                ...
                sendNotification({
                  title: "Setup Success",
                  body: "Auto-start has been enabled for Command Kosh.",
                });
              })
              .catch((e: any) => {
                ...
                sendNotification({
                  title: "Setup Failed",
                  body: "Could not enable auto-start.",
                });
              });
          }}
