...
export function CommandList({ setShowAutostartBanner }: CommandListProps) {
  ...
  const syncCommands = async () => {
    try {
      ...
      const states = await invoke<Record<string, CommandExecutionState>>(
        ...
        {isLoading ? (
          <>
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">
                Please wait while we fetch your commands.
              </p>
            </div>
          </>
        ) : (
          <>
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">
                Create your first command to start scheduling background tasks.
              </p>
            </div>
          </>
        )}
      ...
      <div className="flex flex-col gap-4">
        {commands.map((cmd) => {
          ...
          return (
            <div
              key={cmd.id}
              onClick={() => navigate({ to: `/command-details/${cmd.id}` })}
            >
              <div className="group relative bg-zinc-900/50 border border-zinc-800/60 rounded-xl overflow-hidden cursor-pointer transition-all duration-200 hover:border-zinc-700/80 hover:bg-zinc-900/70 hover:shadow-[0_2px_20px_rgba(0,0,0,0.3)]">
