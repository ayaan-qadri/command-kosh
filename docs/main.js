document.addEventListener('DOMContentLoaded', async () => {
    document.getElementById('copyright').innerHTML = `&copy; ${new Date().getFullYear()} Command Kosh. All rights reserved.`;
    ...
    osNameSpan.textContent = detectedOS !== 'Generic' ? `Download for ${detectedOS}` : 'Download';
    ...
    try {
        ...
        if (data.assets && data.assets.length > 0) {
            ...
            if (detectedOS === 'Windows') {
                const winAsset = assets.find(a => a.name.endsWith('.msi') || a.name.endsWith('x64-setup.exe') || a.name.endsWith('.exe'));
    ...
    document.querySelectorAll('.mockup-wrapper, .feature-text, .feature-img').forEach(el => observers.observe(el));
    ...
    window.addEventListener('scroll', () => {
        ...
        if (swapperBlock) {
            ...
            const isPastMidpoint = rect.top < (window.innerHeight * 0.30);
    ...
    document.querySelectorAll('.mockup-inner img, .feature-img img').forEach(img => {
        img.addEventListener('click', function () {
            currentGallery = Array.from(document.querySelectorAll('.mockup-inner img, .feature-img img')).map(el => el.src);
