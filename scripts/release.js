...
async function bump() {
  ...
  try {
    ...
    const dirtyFiles = execSync(
      `git status --porcelain -- ${trackedFiles.join(" ")}`,
      { encoding: "utf-8" },
    ).trim();
    ...
    originalContents.set(
      tauriConfPath,
      fs.readFileSync(tauriConfPath, "utf-8"),
    );
    originalContents.set(
      cargoTomlPath,
    ...
    if (currentBranch === "master") {
      ...
      console.log(
        `2. The push to 'master' will trigger the automated release workflow.`,
      );
    } else {
